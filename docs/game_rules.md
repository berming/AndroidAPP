...
| **对子** | 2 | 同点数（花色不限，可跨副） |
| **三张** | 3 | 同点数 |
| **顺子** | ≥ 5 | 连续不同点数；**不含大小王**；可以跨 K-A-2 形成循环顺子（如 Q-K-A-2-3） |
| **炸弹** | ≥ 4 | 同点数 |

**顺子细则**（容易踩坑）：

- 顺子的"连续"沿 `3 → 4 → ... → K → A → 2 → 3 → 4 ...` 循环（2 是普通点数 13，不是封顶）
- 合法的 5 连：`3-4-5-6-7` · `10-J-Q-K-A` · `J-Q-K-A-2` · `Q-K-A-2-3` · `K-A-2-3-4` · …
- 不允许在顺子里出现重复点数（即便有多副牌，如 `5-5-6-7-8` 不是合法顺子）
- 不允许大小王进顺子
