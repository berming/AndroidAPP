...
- [ ] **关键路径改动？**（`CardRules` / `SettlementCalculator` / `ServerGameManager`）
  - [ ] 否
  - [ ] 是 —— 必须**全部**满足：
    - [ ] 同 PR 内修改对应 `*Test.kt`（`shared/src/commonTest/.../engine/CardRulesTest.kt` / `SettlementCalculatorTest.kt` / `server/src/test/.../ServerGameManagerTest.kt`）—— **CI tdd-gate 强制校验**
...
- [ ] **服务端状态修改？**（`ServerGameState.hands` / `playerScores` / `currentPlayerIndex`）
  - [ ] 否
  - [ ] 是 —— 已确认：
